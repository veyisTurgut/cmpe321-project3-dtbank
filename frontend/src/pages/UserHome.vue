  ...
  methods: {
    ...
    async fetchResponse() {
      ...
      try {
        ...
        this.loading = true
        if (
          this.section.includes("/drugs") &&
          this.section.includes("/filter")
        ) {
          const response = axios
            .get(
              "http://127.0.0.1:8000" +
                this.section.replaceAll("{", "").replaceAll("}", ""),
              { headers }
            )
            .then(value => {
        ...
        } else if (possible_paths.some(v => this.section.includes(v))) {
          const response = axios
            .get(
              "http://127.0.0.1:8000" +
                this.section.replace("{", "").replace("}", "") +
                "/",
              { headers }
            )
            .then(value => {
              if (this.section.startsWith( "/siders")) {
                /*possible options:
                ...
                */
                ...
                /*possible options:
                  ...
                  #20 "/drugs/{drug_id}/targetss/filter/"
                */
                ...
                /* possible options:
                    ...
                    #14 "/prots/same_drugs", + 
                    #17 "/prots/{uniprot_id}/least_effecting_drug", +
                */
                if (this.section.endsWith("/drugs")) {
                  ...
                  this.posts = value.data.items.map(post => ({
                    ...
                    smiles: post.smiles,
                    description: post.description,
                    number_of_side_effects: post.number_of_side_effects,
                  }))
                }
